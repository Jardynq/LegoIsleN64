...
#include "mx_omni.h"
#include "mx_ram_stream_provider.h"
...
MxResult MxDiskStreamProviderThread::Run() {
	...
	MxThread::Run();
	// They should probably have writen "return MxThread::Run()" but they
	// didn't.
	return SUCCESS;
}
...
MxDiskStreamProvider::~MxDiskStreamProvider() {
	...
	do {
		...
		if (((MxDSStreamingAction*) action)->GetUnknowna0()->GetWriteOffset() <
			0x20000) {
			g_unk0x10102878--;
		}

		((MxDiskStreamController*) m_pLookup)
			->FUN_100c8670((MxDSStreamingAction*) action);
	} while (action);
...
void MxDiskStreamProvider::VTable0x20(MxDSAction* p_action) {
	...
	if (p_action->GetObjectId() == -1) {
		...
		do {
			...
			if (((MxDSStreamingAction*) action)
					->GetUnknowna0()
					->GetWriteOffset() < 0x20000) {
				g_unk0x10102878--;
			}
...
MxResult MxDiskStreamProvider::FUN_100d1780(MxDSStreamingAction* p_action) {
	...
	if (p_action->GetUnknown9c() > 0 && !p_action->GetUnknowna0()) {
		...
		if (buffer->AllocateBuffer(GetFileSize(), MxDSBuffer::e_allocate) !=
			SUCCESS) {
			delete buffer;
			return FAILURE;
		}
...
void MxDiskStreamProvider::PerformWork() {
...

	{
		...
		if (!m_list.empty()) {
			...
			if (streamingAction &&
				!FUN_100d1af0((MxDSStreamingAction*) streamingAction)) {
				m_thread.Sleep(500);
				m_busySemaphore.Release(1);
				return;
			}
	...
	if (((MxDSStreamingAction*) streamingAction)
			->GetUnknowna0()
			->GetWriteOffset() < 0x20000) {
		g_unk0x10102878--;
	}
	...
	if (m_pFile->GetPosition() ==
			((MxDSStreamingAction*) streamingAction)->GetBufferOffset() ||
		m_pFile->Seek(
			((MxDSStreamingAction*) streamingAction)->GetBufferOffset(),
			SEEK_SET
		) == 0) {
		...
		if (m_pFile->ReadToBuffer(buffer) == SUCCESS) {
			...
			if (((MxDSStreamingAction*) streamingAction)->GetUnknown9c() > 0) {
				...
				if (m_pLookup == NULL ||
					!((MxDiskStreamController*) m_pLookup)->GetUnk0xc4()) {
					controller->FUN_100c8670(
						((MxDSStreamingAction*) streamingAction)
					);
				} else {
					controller->FUN_100c7f40(
						((MxDSStreamingAction*) streamingAction)
					);
				}
			}
...
MxResult MxDiskStreamProvider::FUN_100d1b20(MxDSStreamingAction* p_action) {
	...
	MxU32 size =
		(p_action->GetUnknowna4() ? p_action->GetUnknowna4()->GetWriteOffset()
								  : 0) +
	...
	while (data) {
		if (*IntoType(data) != FOURCC('M', 'x', 'O', 'b') &&
			...
			*(MxU32*) (data + 4) =
			...
			memcpy(
				buffer->GetBuffer(),
				p_action->GetUnknowna4()->GetBuffer(),
				size
			);
