...
MxLong JukeBoxEntity::Notify(MxParam& p_param) {
	...
	if (param.GetNotification() == c_notificationClick) {
		...
		((Isle*) FindWorld(*g_isleScript, IsleScript::c__Isle))
			->SetDestLocation(LegoGameState::e_jukeboxw);
		TransitionManager()
