...
IsleScript::Script g_cptClickDialogue[] = {
	IsleScript::c_Avo905Ps_PlayWav,
	IsleScript::c_Avo906Ps_PlayWav,
	IsleScript::c_Avo907Ps_PlayWav};
	...
	default:
		...
		if (result == 0) {
			...
			if (action->GetAtomId() == *g_jukeboxScript) {
				...
				if (script >= JukeboxScript::c_JBMusic1 &&
					script <= JukeboxScript::c_JBMusic6) {
				...
				if (script >= IsleScript::c_nic002pr_RunAnim &&
					script <= IsleScript::c_nic004pr_RunAnim) {
				...
				if (script == IsleScript::c_Avo917In_PlayWav ||
					(script >= IsleScript::c_Avo900Ps_PlayWav &&
					 script <= IsleScript::c_Avo907Ps_PlayWav)) {
...
void Isle::UpdateGlobe() {
	...
	for (MxS32 i = 0; i < 6; i++) {
		MxStillPresenter* presenter = (MxStillPresenter*)
			Find(*g_isleScript, IsleScript::c_Observe_Globe1_Bitmap + i);
...
void Isle::Enable(MxBool p_enable) {
	...
	if (p_enable) {
		...
		if (UserActor() != NULL &&
			UserActor()->GetActorId() != LegoActor::c_none) {
		...
		case LegoGameState::e_elevride:
			...
			NotificationManager()->Send(
				this,
				MxNotificationParam(c_notificationTransitioned, NULL)
			);
			SetIsWorldActive(FALSE);
			break;
		case LegoGameState::e_jetrace2:
			if (((JetskiRaceState*) GameState()->GetState("JetskiRaceState"))
					->m_unk0x28 == 2) {
		...
		if (m_act1state->m_unk0x018 != 8 &&
			...
			(m_act1state->m_unk0x018 != 0 ||
			 GameState()->m_currentArea != LegoGameState::e_skateboard) &&
			(m_act1state->m_unk0x018 != 0 ||
			 GameState()->m_currentArea != LegoGameState::e_jetrace2)) {
			FUN_10015820(
				FALSE,
				LegoOmni::c_disableInput | LegoOmni::c_disable3d |
					LegoOmni::c_clearScreen
...
MxLong Isle::HandleTransitionEnd() {
	...
	case LegoGameState::e_infomain:
		((LegoEntity*) Find(*g_isleScript, IsleScript::c_InfoCenter_Entity))
			->GetROI()
			->SetVisibility(TRUE);
	...
	case LegoGameState::e_bike:
		m_act1state->m_unk0x01f = TRUE;
		FUN_10032d30(
			IsleScript::c_BikeDashboard_Bitmap,
	...
	case LegoGameState::e_copter:
		m_act1state->m_unk0x01f = TRUE;
		FUN_10032d30(
			IsleScript::c_HelicopterDashboard_Bitmap,
			JukeboxScript::c_MusicTheme1,
			NULL,
			TRUE
		);
	...
	case LegoGameState::e_skateboard:
		m_act1state->m_unk0x01f = TRUE;
		FUN_10032d30(
			IsleScript::c_SkatePizza_Bitmap,
			JukeboxScript::c_MusicTheme1,
			NULL,
			TRUE
		);
	...
	case LegoGameState::e_ambulance:
		...
		m_act1state->m_unk0x018 = 10;
		FUN_10032d30(
			IsleScript::c_AmbulanceFuelMeter,
			JukeboxScript::c_MusicTheme1,
			NULL,
			TRUE
		);
	...
	case LegoGameState::e_jetski:
		m_act1state->m_unk0x01f = TRUE;
		FUN_10032d30(
			(IsleScript::Script) m_jetski->GetUnknown0x160(),
			JukeboxScript::c_MusicTheme1,
			NULL,
			TRUE
		);
...
void Isle::FUN_10032d30(
	...
	if (m_act1state->m_unk0x01f) {
		...
		if (presenter != NULL &&
			presenter->GetCurrentTickleState() == MxPresenter::e_repeating) {
...
MxBool Isle::Escape() {
	...
	DeleteObjects(
		&m_atomId,
		IsleScript::c_sba001bu_RunAnim,
		IsleScript::c_FNS018EN_Wav_518
	);
...
Act1State::Act1State() {
	...
	m_unk0x018 = 1;
	m_unk0x01e = FALSE;
	m_cptClickDialogue = Playlist(
		(MxU32*) g_cptClickDialogue,
		sizeOfArray(g_cptClickDialogue),
		Playlist::e_loop
	);
	m_unk0x01f = FALSE;
	m_planeActive = FALSE;
...
MxResult Act1State::Serialize(LegoStorage* p_storage) {
	...
	if (p_storage->IsWriteMode()) {
		...
		if (strcmp(m_helicopterPlane.m_name.GetData(), "")) {
			if ((m_helicopterWindshield = ReadNamedTexture(p_storage)) ==
				NULL) {
				return FAILURE;
			}
...
void Act1State::PlaceActors() {
	...
	if (m_jetski != NULL) {
		if (!m_jetskiPlane.IsPresent()) {
			...
			isle->PlaceActor(
				m_jetski,
				m_jetskiPlane.GetName(),
				0,
				0.5f,
				1,
				0.5f
			);
			m_jetski->SetLocation(
				m_jetskiPlane.GetPosition(),
				m_jetskiPlane.GetDirection(),
				m_jetskiPlane.GetUp(),
				TRUE
			);
			isle->Add(m_jetski);
