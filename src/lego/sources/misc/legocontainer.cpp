...
LegoTextureInfo* LegoTextureContainer::GetCached(LegoTextureInfo* p_textureInfo
	...
	for (LegoCachedTextureList::iterator it = m_cached.begin();
		 it != m_cached.end();
		 it++) {
		if ((*it).second == FALSE && (*it).first->m_texture->AddRef() != 0 &&
			(*it).first->m_texture->Release() == 1) {
			if (!strcmp((*it).first->m_name, p_textureInfo->m_name)) {
				...
				if (surface
						->Lock(NULL, &newDesc, DDLOCK_SURFACEMEMORYPTR, NULL) ==
					DD_OK) {
					BOOL und = FALSE;
					if (newDesc.dwWidth == width &&
						newDesc.dwHeight == height) {
						und = TRUE;
					}
	...
	if (VideoManager()->GetDirect3D()->DirectDraw()->CreateSurface(
			&newDesc,
		...
		if (textureInfo->m_surface->BltFast(
				0,
				0,
				p_textureInfo->m_surface,
				&rect,
				DDBLTFAST_WAIT
			) != DD_OK) {
			...
			if (((TglImpl::RendererImpl*) VideoManager()->GetRenderer())
				...
				textureInfo->m_name =
					new char[strlen(p_textureInfo->m_name) + 1];
