...
Result MeshImpl::SetColor(float r, float g, float b, float a) {
	...
	if (*reinterpret_cast<int*>(&a) > 0) {
		...
		return ResultVal(
			m_data->groupMesh->SetGroupColor(m_data->groupIndex, color)
		);
	} else {
		return ResultVal(
			m_data->groupMesh->SetGroupColorRGB(m_data->groupIndex, r, g, b)
		);
	}
}
...
Result MeshImpl::SetTexture(const Texture* pTexture) {
	IDirect3DRMTexture* texture =
		pTexture
			...
			: NULL;
	return ResultVal(
		m_data->groupMesh->SetGroupTexture(m_data->groupIndex, texture)
	);
}
...
Result MeshImpl::SetTextureMappingMode(TextureMappingMode mode) {
	if (mode == PerspectiveCorrect) {
		...
		return ResultVal(
			m_data->groupMesh->SetGroupMapping(m_data->groupIndex, 0)
		);
	}
}
...
Result MeshImpl::SetShadingModel(ShadingModel model) {
	...
	return ResultVal(
		m_data->groupMesh->SetGroupQuality(m_data->groupIndex, mode)
	);
}
...
Mesh* MeshImpl::DeepClone(MeshBuilder* pMeshBuilder) {
	...
	D3DRMMAPPING mapping =
		m_data->groupMesh->GetGroupMapping(m_data->groupIndex);
	D3DRMRENDERQUALITY quality =
		m_data->groupMesh->GetGroupQuality(m_data->groupIndex);
	...
	target->ImplementationData()
		->AddGroup(vcount, fcount, vperface, faceBuffer, &index);
	...
	Result result =
		ResultVal(target->ImplementationData()->SetGroupColor(index, color));
...
Mesh* MeshImpl::ShallowClone(MeshBuilder* pMeshBuilder) {
	...
	if (newData) {
		...
		newData->groupMesh =
			static_cast<MeshBuilderImpl*>(pMeshBuilder)->ImplementationData();
	} else {
		delete newGroup;
		newGroup = NULL;
	}
...
Result MeshImpl::GetTexture(Texture*& rpTexture) {
	...
	Result result = ResultVal(
		m_data->groupMesh->GetGroupTexture(m_data->groupIndex, &texture)
	);
	if (result) {
