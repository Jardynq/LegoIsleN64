	...
	if ((key < 'A' || key > 'Z') && key != VK_BACK) {
		if (key == VK_SPACE) {
			DeleteObjects(
				...
				RegbookScript::c_iic008in_PlayWav
			);
			BackgroundAudioManager()->RaiseVolume();
		}
		...
		if (m_name[0][m_unk0x280.m_cursorPos] != NULL) {
			m_alphabet[key - 'A']->GetAction()->SetUnknown24(
				m_alphabet[key - 'A']->GetAction()->GetUnknown24() + 1
			);
			m_name[0][m_unk0x280.m_cursorPos]->Enable(TRUE);
			m_name[0][m_unk0x280.m_cursorPos]->SetTickleState(
				MxPresenter::e_repeating
			);
			m_name[0][m_unk0x280.m_cursorPos]->SetPosition(
				m_unk0x280.m_cursorPos * 23 + 343,
				121
			);
...
RegistrationBook::HandleControl(LegoControlManagerNotificationParam& p_param) {
	...
	if (unk0x28 >= 1 && unk0x28 <= 28) {
		if (p_param.GetClickedObjectId() == RegbookScript::c_Alphabet_Ctl) {
			...
			MxS16 i;
			for (i = 0; i < 10; i++) {
				if (m_checkmark[i]->GetAction()->GetObjectId() ==
					p_param.GetClickedObjectId()) {
					break;
				}
			}
...
void RegistrationBook::FUN_100775c0(MxS16 p_playerIndex) {
	...
	MxS16 player = p_playerIndex == 0
					   ? GameState()->FindPlayer(
							 *(LegoGameState::Username*) &m_unk0x280.m_letters
						 )
					   : p_playerIndex - 1;
	...
	if (GameState()->GetCurrentAct() == LegoGameState::e_act1) {
		...
		if (act1state->m_helicopterPlane.IsPresent()) {
			InvokeAction(
				Extra::e_start,
				m_atomId,
				CopterScript::c_Helicopter_Actor,
				NULL
			);
		...
		if (act1state->m_dunebuggyPlane.IsPresent()) {
			InvokeAction(
				Extra::e_start,
				m_atomId,
				DunecarScript::c_DuneBugy_Actor,
				NULL
			);
			NotificationManager()->Send(
				this,
...
void RegistrationBook::ReadyWorld() {
	...
	// `assert()` calls, which shouldn't affect release builds at all. See
	// https://github.com/isledecomp/isle/pull/1375 for a version that had 100
	// %.
	...
	for (i = 0; i < 26; i++) {
		...
		if (i < 26) {
			m_alphabet[i] =
				(MxStillPresenter*) Find("MxStillPresenter", letterBuffer);
			assert(m_alphabet[i]);
	...
	for (i = 0; i < 10; i++) {
		m_checkmark[i] =
			(MxControlPresenter*) Find("MxControlPresenter", checkmarkBuffer);
...
MxResult RegistrationBook::Tickle() {
	if (!m_worldStarted) {
		...
		if (m_registerDialogueTimer != 0x80000000 &&
			m_registerDialogueTimer + 30000 <= time) {
		...
		if (g_checkboxBlinkTimer + 500 <= time) {
			...
			if (m_checkboxHilite) {
				...
				if (g_nextCheckbox) {
					m_checkboxSurface
						->Blt(NULL, m_checkboxHilite, NULL, DDBLT_ROP, &op);
				} else {
					m_checkboxSurface
						->Blt(NULL, m_checkboxNormal, NULL, DDBLT_ROP, &op);
				}
			} else {
				CreateSurface();
			}
...
RegistrationBook::HandlePathStruct(LegoPathStructNotificationParam& p_param) {
	...
	case CopterScript::c_Helicopter_Actor:
		actor =
			(LegoPathActor*) Find(m_atomId, CopterScript::c_Helicopter_Actor);
	...
	case DunecarScript::c_DuneBugy_Actor:
		actor =
			(LegoPathActor*) Find(m_atomId, DunecarScript::c_DuneBugy_Actor);
...
MxBool RegistrationBook::CreateSurface() {
	...
	if (presenters) {
		...
		presenter =
			(MxStillPresenter*) Find("MxStillPresenter", "CheckHiLite_Bitmap");
		if (presenter) {
