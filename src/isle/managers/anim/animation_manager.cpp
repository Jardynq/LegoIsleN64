...
LegoAnimationManager::~LegoAnimationManager() {
	...
	for (MxS32 i = 0; i < (MxS32) sizeOfArray(m_extras); i++) {
		...
		if (roi != NULL) {
			LegoPathActor* actor =
				CharacterManager()->GetExtraActor(roi->GetName());
	...
	if (!m_suspended) {
		...
		for (i = 0; i < (MxS32) sizeOfArray(m_extras); i++) {
			...
			if (m_extras[i].m_unk0x14) {
				...
				MxS32 vehicleId =
					g_characters[m_extras[i].m_characterId].m_vehicleId;
				if (vehicleId >= 0) {
					g_vehicles[vehicleId].m_unk0x05 = FALSE;

					LegoROI* roi =
						Lego()->FindROI(g_vehicles[vehicleId].m_name);
					if (roi != NULL) {
...
MxResult LegoAnimationManager::LoadWorldInfo(LegoOmni::World p_worldId) {
	...
	if (m_worldId != p_worldId) {
		...
		char path[128];
		sprintf(path, "rom:/data/%sinf.dta", Lego()->GetWorldName(p_worldId));
		...
		for (j = 0; j < m_animCount; j++) {
			...
			m_anims[j].character_index = GetCharacterIndex(
				m_anims[j].m_name + strlen(m_anims[j].m_name) - 2
			);
			...
			if (m_anims[j].m_location == -1) {
				for (MxS32 l = 0; l < m_anims[j].m_modelCount; l++) {
					MxS32 index =
						GetCharacterIndex(m_anims[j].m_models[l].m_name);
			...
			for (MxS32 m = 0; m < m_anims[j].m_modelCount; m++) {
				...
				if (FindVehicle(m_anims[j].m_models[m].m_name, n) &&
					m_anims[j].m_models[m].m_unk0x2c) {
...
LegoAnimationManager::ReadAnimInfo(LegoStorage* p_storage, AnimInfo* p_info) {
	...
	if (p_storage->Read(&p_info->m_objectId, sizeof(p_info->m_objectId)) ==
		FAILURE) {
		goto done;
	}

	if (p_storage->Read(&p_info->m_location, sizeof(p_info->m_location)) ==
		FAILURE) {
		goto done;
	}
	if (p_storage->Read(&p_info->m_unk0x0a, sizeof(p_info->m_unk0x0a)) ==
		FAILURE) {
		goto done;
	}
	if (p_storage->Read(&p_info->m_unk0x0b, sizeof(p_info->m_unk0x0b)) ==
		FAILURE) {
		goto done;
	}
	if (p_storage->Read(&p_info->m_unk0x0c, sizeof(p_info->m_unk0x0c)) ==
		FAILURE) {
		goto done;
	}
	if (p_storage->Read(&p_info->m_unk0x0d, sizeof(p_info->m_unk0x0d)) ==
		FAILURE) {
		goto done;
	}
	...
	for (i = 0; i < (MxS32) sizeOfArray(p_info->m_unk0x10); i++) {
		if (p_storage->Read(
				&p_info->m_unk0x10[i],
				sizeof(*p_info->m_unk0x10)
			) != SUCCESS) {
			goto done;
		}
	}

	if (p_storage->Read(&p_info->m_modelCount, sizeof(p_info->m_modelCount)) ==
		FAILURE) {
		goto done;
	}
	...
	memset(
		p_info->m_models,
		0,
		p_info->m_modelCount * sizeof(*p_info->m_models)
	);
...
LegoAnimationManager::ReadModelInfo(LegoStorage* p_storage, ModelInfo* p_info) {
	...
	p_info->m_name[length] = 0;
	if (p_storage->Read(&p_info->m_unk0x04, sizeof(p_info->m_unk0x04)) ==
		FAILURE) {
		goto done;
	}
	...
	if (p_storage->Read(p_info->m_direction, sizeof(p_info->m_direction)) !=
		SUCCESS) {
		goto done;
	}
	...
	if (p_storage->Read(&p_info->m_unk0x2c, sizeof(p_info->m_unk0x2c)) ==
		FAILURE) {
		goto done;
	}
...
MxResult LegoAnimationManager::FUN_100605e0(
	...
	if (m_worldId != LegoOmni::e_undefined && p_index < m_animCount &&
		m_tranInfoList != NULL) {
		PurgeExtra(FALSE);
		FUN_10062770();
...
MxResult LegoAnimationManager::StartEntityAction(
	...
	if (LegoOmni::GetInstance()->StartActionIfUnknown0x13c(p_dsAction) ==
		SUCCESS) {
		result = SUCCESS;
	}
	...
	if (Lego()->m_unk0x13c && m_enableCamAnims && !m_animRunning) {
		...
		if (FUN_100617c0(p_location, unk0x0e, unk0x10) == SUCCESS) {
			...
			for (MxU16 i = unk0x0e; i <= unk0x10; i++) {
				...
				if ((p_bool || !FUN_100623a0(animInfo)) &&
					!FUN_10062710(animInfo) && animInfo.m_unk0x29 &&
					animInfo.m_unk0x22 < unk0x22 &&
					(animInfo.m_unk0x22 == 0 || *animInfo.m_name != 'i') &&
					*animInfo.m_name != 'I') {
					index = i;
			...
			if (tranInfo->m_presenter != NULL) {
				...
				if (tranInfo->m_unk0x14 && tranInfo->m_location != -1 &&
					...
					if (tranInfo->m_presenter->GetPresenter() != NULL &&
						(anim = tranInfo->m_presenter->GetPresenter()
									->GetAnimation()) != NULL &&
...
MxResult LegoAnimationManager::FUN_100617c0(
	...
	if (p_location == -1) {
		...
		if (success) {
			for (; i < m_animCount && m_anims[i].m_location == p_location;
				 i++) {
				unk0x10 = i;
			}
		}
...
MxLong LegoAnimationManager::Notify(MxParam& p_param) {
	...
	} else if (((MxNotificationParam&) p_param).GetNotification() ==
				   c_notificationEndAnim &&
			   m_tranInfoList != NULL) {
		LegoTranInfoListCursor cursor(m_tranInfoList);
		LegoTranInfo* tranInfo;
...
MxResult LegoAnimationManager::Tickle() {
	...
	if (!m_animRunning && time - m_unk0x404 > 10000 &&
		speed < g_unk0x100f74b0[0][0] && speed > g_unk0x100f74b0[5][0]) {
...
MxU16 LegoAnimationManager::FUN_10062110(
	...
	if (actor != NULL && actor->GetBoundary() == p_boundary &&
		actor->GetActorState() == LegoPathActor::c_initial) {
			...
			if (direction.Dot(direction, p_direction) > 0.707) {
				...
				if (len < max && len > min) {
					...
					for (MxU16 i = m_unk0x0e; i <= m_unk0x10; i++) {
						if (m_anims[i].character_index == index &&
							m_anims[i].m_unk0x0c & p_unk0x0c &&
							m_anims[i].m_unk0x29) {
							...
							if (vehicleId >= 0) {
								...
								for (MxS32 j = 0;
									 j <
									 (MxS32) sizeOfArray(m_anims[i].m_unk0x2a);
									 j++) {
...
MxS8 LegoAnimationManager::GetCharacterIndex(const char* p_name) {
	...
	for (i = 0; i < sizeOfArray(g_characters); i++) {
		if (!strncmpi(p_name, g_characters[i].m_name, 2)) {
			return i;
		}
	}
...
void LegoAnimationManager::FUN_10062580(AnimInfo& p_info) {
	...
	if (models != NULL && modelCount) {
		for (MxU8 i = 0; i < modelCount; i++) {
			LegoPathActor* actor =
				CharacterManager()->GetExtraActor(models[i].m_name);
				...
				if (controller) {
					...
					for (MxS32 i = 0; i < (MxS32) sizeOfArray(m_extras); i++) {
						if (m_extras[i].m_roi == actor->GetROI()) {
							...
							g_characters[characterId].m_unk0x07 = TRUE;
							MxS32 vehicleId =
								g_characters[characterId].m_vehicleId;
...
void LegoAnimationManager::FUN_10062770() {
	if (!m_unk0x38) {
		...
		if (world != NULL) {
			m_unk0x28[1] = (MxPresenter*
			) world->Find("MxSoundPresenter", "TransitionSound1");
			m_unk0x28[0] = (MxPresenter*
			) world->Find("MxSoundPresenter", "TransitionSound2");
			m_unk0x30[1] = 200;
			m_unk0x30[0] = 750;
			m_unk0x38 = TRUE;
		}
	...
	if (p_und || viewManager != NULL) {
		...
		for (MxS32 i = 0; i < (MxS32) sizeOfArray(m_extras); i++) {
			...
			if (roi != NULL) {
				...
				MxBool maOrPa =
					prefix == TWOCC('m', 'a') || prefix == TWOCC('p', 'a');

				if ((p_und && !maOrPa) ||
					(g_characters[m_extras[i].m_characterId].m_unk0x10 >= 0 &&
					 time - m_extras[i].m_unk0x08 > und &&
					 CharacterManager()->GetRefCount(roi) == 1 &&
					 !viewManager->IsBoundingBoxInFrustum(
						 roi->GetWorldBoundingBox()
					 ))) {
					m_unk0x414--;
	...
	if (p_und || (!m_animRunning && m_unk0x400)) {
		...
		if (world != NULL) {
			...
			if (actor == NULL || actor->GetWorldSpeed() <= 20.0f) {
				MxU32 i;
				for (i = 0; i < m_numAllowedExtras && m_extras[i].m_roi != NULL;
					 i++) {
				}
				...
				if (i != m_numAllowedExtras) {
					...
					if (p_location < 0) {
						...
						LegoLocation* location =
							LegoNavController::GetLocation(p_location);
					...
					if (boundary != NULL) {
						for (i = 0; i < m_numAllowedExtras; i++) {
							if (m_extras[i].m_roi == NULL) {
								...
								if (m_lastExtraCharacterId >=
									sizeOfArray(g_characters)) {
									m_lastExtraCharacterId = 0;
								}
							...
							tryNextCharacter:
								if (g_characters[m_lastExtraCharacterId]
										.m_unk0x09 &&
									g_characters[m_lastExtraCharacterId]
										.m_unk0x08 &&
									!g_characters[m_lastExtraCharacterId]
										 .m_inExtras &&
									g_characters[m_lastExtraCharacterId]
									...
									if (!CharacterManager()->Exists(
										...
										LegoExtraActor* actor =
											CharacterManager()->GetExtraActor(
												g_characters
													[m_lastExtraCharacterId]
														.m_name
											);
										...
										if (world->PlaceActor(
											...
											m_extras[i].m_characterId =
												m_lastExtraCharacterId;
											g_characters[m_lastExtraCharacterId]
												.m_inExtras = TRUE;
											m_extras[i].m_unk0x08 =
												Timer()->GetTime();
	...
	if (actor != NULL) {
		...
		if (!local24) {
			...
			if (FUN_10063b90(
					world,
					actor,
					CharacterManager()->GetMood(p_roi),
					characterId
				)) {
				m_extras[i].m_unk0x14 = TRUE;
			} else {
					...
					for (MxS32 i = 0; i < (MxS32) sizeOfArray(m_extras); i++) {
						if (m_extras[i].m_roi == roi) {
							...
							if (m_extras[i].m_unk0x14) {
								...
								MxS32 vehicleId =
									g_characters[m_extras[i].m_characterId]
										.m_vehicleId;
								if (vehicleId >= 0) {
									g_vehicles[vehicleId].m_unk0x05 = FALSE;

									LegoROI* roi = Lego()->FindROI(
										g_vehicles[vehicleId].m_name
									);
									if (roi != NULL) {
										roi->SetVisibility(FALSE);
									}
							...
							m_extras[i].m_roi = NULL;
							g_characters[m_extras[i].m_characterId].m_inExtras =
								FALSE;
							g_characters[m_extras[i].m_characterId].m_unk0x07 =
								FALSE;
							m_extras[i].m_characterId = -1;
							m_extras[i].m_unk0x0d = FALSE;
							m_unk0x414--;
...
MxBool LegoAnimationManager::FUN_10063b90(
	...
	if (g_characters[p_characterId].m_vehicleId >= 0 &&
		g_vehicles[g_characters[p_characterId].m_vehicleId].m_unk0x04 &&
		(vehicleWC = cycles[10]) != NULL) {
		presenter = (LegoLocomotionAnimPresenter*
		) p_world->Find("LegoAnimPresenter", vehicleWC);
...
void LegoAnimationManager::FUN_10063d10() {
	if (CurrentWorld() != NULL) {
		...
		for (MxS32 i = 0; i < (MxS32) sizeOfArray(m_extras); i++) {
			...
			if (roi != NULL) {
				if (m_extras[i].m_unk0x0c &&
					g_characters[m_extras[i].m_characterId].m_unk0x0c >= 0 &&
					g_characters[m_extras[i].m_characterId].m_unk0x0c <
						time - m_extras[i].m_unk0x08) {
					...
					if (actor != NULL) {
						...
						if (speed < 0.0f) {
							if (m_extras[i].m_unk0x14) {
								speed =
									((float) (rand() * 1.5) / 32767.0f) + 0.9;
							} else {
								speed =
									((float) (rand() * 1.4) / 32767.0f) + 0.6;
							}
						}
...
MxBool LegoAnimationManager::FUN_10063fb0(
	...
	if (p_boundary->m_name != NULL) {
		Mx3DPointFloat vec;
		LegoPathBoundary* boundary =
			p_world->FindPathBoundary(p_boundary->m_name);
		LegoUnknown100db7f4* pSrcE =
			(LegoUnknown100db7f4*) boundary->GetEdges()[p_boundary->m_src];
...
MxBool LegoAnimationManager::FUN_10064120(
	...
	while (local2c--) {
		if (local34 != NULL) {
			if (local34->BETA_1004a830(*boundary, LegoWEGEdge::c_bit1) &&
				FUN_10064010(boundary, local34, destScale) &&
				...
				for (i = 0; i < numEdges; i++) {
					LegoUnknown100db7f4* e =
						(LegoUnknown100db7f4*) boundary->GetEdges()[i];
			...
			do {
				if (p_bool1) {
					local34 = (LegoUnknown100db7f4*
					) local34->GetCounterclockwiseEdge(*boundary);
				} else {
					local34 = (LegoUnknown100db7f4*
					) local34->GetClockwiseEdge(*boundary);
				}
...
MxResult LegoAnimationManager::FUN_10064380(
	...
	if (actor == NULL && extraIndex != -1) {
		...
		MxS32 characterId;
		for (characterId = 0; characterId < (MxS32) sizeOfArray(g_characters);
			 characterId++) {
		...
		m_extras[extraIndex].m_roi =
			CharacterManager()->GetActorROI(p_name, TRUE);
	...
	if (actor != NULL) {
		...
		LegoLocomotionAnimPresenter* presenter = (LegoLocomotionAnimPresenter*
		) world->Find("LegoAnimPresenter", cycles[p_undIdx1]);
		if (presenter != NULL) {
			presenter->FUN_1006d680(actor, 0.0f);
		}

		presenter = (LegoLocomotionAnimPresenter*
		) world->Find("LegoAnimPresenter", cycles[p_undIdx2]);
		if (presenter != NULL) {
...
void LegoAnimationManager::FUN_100648f0(
	...
	if (m_unk0x402 && p_tranInfo->m_unk0x14) {
		...
		LegoLocation* location =
			NavController()->GetLocation(p_tranInfo->m_location);
		if (location != NULL) {
			CalcLocalTransform(
				location->m_position,
				location->m_direction,
				location->m_up,
				m_unk0x484
			);
...
void LegoAnimationManager::FUN_10064b50(MxLong p_time) {
	if (m_unk0x430 && m_unk0x42c != NULL) {
		...
		if (p_time < 0 || m_unk0x438 <= p_time) {
			...
			float und = (float) (p_time - m_unk0x434) /
						(float) (m_unk0x438 - m_unk0x434);
			...
			m_unk0x4cc.InterpolateToMatrix(
				mat,
				(float) (p_time - m_unk0x434) / 1000.0f
			);
...
MxResult AnimState::Serialize(LegoStorage* p_storage) {
	...
	if (result == SUCCESS) {
		if (p_storage->IsReadMode()) {
			...
			// Note that here we read first and then free memory in contrast to
			// above
