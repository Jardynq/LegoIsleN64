...
public:
	...
	Element(int p_row, int p_col) const;                    // vtable+0x18
	...
	inline virtual void TranslateBy(
		const float& p_x,
		const float& p_y,
		const float& p_z
	); // vtable+0x30
	...
	inline virtual void
	Product(float (*p_a)[4], float (*p_b)[4]); // vtable+0x3c
	inline virtual void
	Product(const Matrix4& p_a, const Matrix4& p_b);         // vtable+0x38
