...
Point Rect::GetPoint() const {
	return Point(this->m_left, this->m_top);
}
Size Rect::GetSize() const {
	return Size(this->m_right, this->m_bottom);
}

i32 Rect::GetLeft() const {
	return m_left;
}
i32 Rect::GetTop() const {
	return m_top;
}
i32 Rect::GetRight() const {
	return m_right;
}
i32 Rect::GetBottom() const {
	return m_bottom;
}

void Rect::SetLeft(i32 p_left) {
	m_left = p_left;
}
void Rect::SetTop(i32 p_top) {
	m_top = p_top;
}
void Rect::SetRight(i32 p_right) {
	m_right = p_right;
}
void Rect::SetBottom(i32 p_bottom) {
	m_bottom = p_bottom;
}
