...
int LegoDeviceEnumerate::ProcessDeviceBytes(int p_deviceNum, GUID& p_guid) {
	...
	for (list<MxDriver>::iterator it = m_list.begin(); it != m_list.end();
		...
		GUID4 compareGuid;
		MxDriver& driver = *it;
		for (list<Direct3DDeviceInfo>::iterator it2 = driver.m_devices.begin();
...
int LegoDeviceEnumerate::GetDevice(
	...
	for (list<MxDriver>::iterator it = m_list.begin(); it != m_list.end();
		...
		for (list<Direct3DDeviceInfo>::iterator it2 =
				 p_driver->m_devices.begin();
			 it2 != p_driver->m_devices.end();
			 it2++) {
			if (i == p_deviceNum) {
				p_device = &*it2;
				return 0;
...
int LegoDeviceEnumerate::FormatDeviceName(
	...
	for (list<MxDriver>::const_iterator it = m_list.begin(); it != m_list.end();
		 it++, number++) {
		if (&(*it) == p_ddInfo) {
			GUID4 guid;
	...
	for (list<MxDriver>::iterator it = m_list.begin(); it != m_list.end();
		...
		for (list<Direct3DDeviceInfo>::iterator it2 = driver.m_devices.begin();
			...
			if ((*it2).m_HWDesc.dcmColorModel) {
				...
				if (cpu_mmx && (*it2).m_HELDesc.dcmColorModel == D3DCOLOR_RGB &&
					i == 0) {
					k = j;
...
#else
	__asm__(
		"movl $0x0, %%eax\n\t" // EAX=0: Highest Function Parameter and
							   // Manufacturer ID
		"cpuid\n\t"            // Run CPUID\n"
		"mov $0x1, %%eax\n\t" // EAX=1: Processor Info and Feature Bits (unused)
		"cpuid\n\t"           // Run CPUID
		"xorl %%eax, %%eax\n\t" // Zero EAX register
		"btl $0x15, %%edx\n\t"  // Test bit 0x17 (23): MMX instructions (64-bit
								// SIMD) (Store in CF)
		"adc %%eax, %%eax"      // Add with carry: EAX = EAX + EAX + CF = CF
...
int LegoDeviceEnumerate::SupportsCPUID() {
	...
	__asm__("xorl %%eax, %%eax\n\t" // Zero EAX register
			...
			"orl $0x200000, (%%esp)\n\t" // Set bit 0x200000: Able to use CPUID
										 // instruction (Pentium+)
			...
			"popfl" // Push EFLAGS register value on the stack (again, and makes
					// sure the stack remains the same)
...
LegoDeviceEnumerate::DriverSupportsRequiredDisplayMode(MxDriver& p_driver) {
	for (list<MxDisplayMode>::iterator it = p_driver.m_displayModes.begin();
		 it != p_driver.m_displayModes.end();
		 it++) {
...
unsigned char LegoDeviceEnumerate::FUN_1009d3d0(Direct3DDeviceInfo& p_device) {
	...
	if (p_device.m_HWDesc.dcmColorModel) {
		if (p_device.m_HWDesc.dwDeviceZBufferBitDepth & DDBD_16 &&
			p_device.m_HWDesc.dpcTriCaps.dwTextureCaps &
	...
	for (list<Direct3DDeviceInfo>::iterator it = front.m_devices.begin();
		 it != front.m_devices.end();
		 it++) {
		if ((&*it) == &p_device) {
			return TRUE;
		}
