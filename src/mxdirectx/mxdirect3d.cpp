...
void printDDSurfaceDesc(const DDSURFACEDESC* desc) {
	...
	printf("  Color Keys:\n");
	printf(
		"    Dest Overlay:      Low: 0x%08X  High: 0x%08X\n",
		desc->ddckCKDestOverlay.dwColorSpaceLowValue,
		desc->ddckCKDestOverlay.dwColorSpaceHighValue
	);
	printf(
		"    Dest Blt:          Low: 0x%08X  High: 0x%08X\n",
		desc->ddckCKDestBlt.dwColorSpaceLowValue,
		desc->ddckCKDestBlt.dwColorSpaceHighValue
	);
	printf(
		"    Src Overlay:       Low: 0x%08X  High: 0x%08X\n",
		desc->ddckCKSrcOverlay.dwColorSpaceLowValue,
		desc->ddckCKSrcOverlay.dwColorSpaceHighValue
	);
	printf(
		"    Src Blt:           Low: 0x%08X  High: 0x%08X\n",
...
void printDDSCaps(DWORD dwCaps) {
	...
	if (dwCaps & 0x00000001)
		printf("    - DDSCAPS_ALPHA: Surface contains alpha channel\n");
	if (dwCaps & 0x00000002)
		printf("    - DDSCAPS_BACKBUFFER: Surface is a back buffer\n");
	if (dwCaps & 0x00000004)
		printf("    - DDSCAPS_COMPLEX: Surface is complex (e.g., mipmaps)\n");
	if (dwCaps & 0x00000008)
		printf("    - DDSCAPS_FLIP: Surface supports flipping\n");
	if (dwCaps & 0x00000010)
		printf("    - DDSCAPS_FRONTBUFFER: Surface is a front buffer\n");
	if (dwCaps & 0x00000020)
		printf("    - DDSCAPS_OFFSCREENPLAIN: Offscreen surface\n");
	if (dwCaps & 0x00000040)
		printf("    - DDSCAPS_OVERLAY: Surface is an overlay\n");
	if (dwCaps & 0x00000080)
		printf("    - DDSCAPS_PALETTE: Surface has an attached palette\n");
	if (dwCaps & 0x00000100)
		printf("    - DDSCAPS_PRIMARYSURFACE: Surface is the primary display\n"
		);
	if (dwCaps & 0x00000200)
		printf(
			"    - DDSCAPS_SYSTEMMEMORY: Surface is stored in system memory\n"
		);
	if (dwCaps & 0x00000400)
		printf("    - DDSCAPS_TEXTURE: Surface is used as a texture\n");
	if (dwCaps & 0x00000800)
		printf("    - DDSCAPS_VIDEOMEMORY: Surface is stored in video memory\n"
		);
	if (dwCaps & 0x00001000)
		printf("    - DDSCAPS_WRITEONLY: Surface is write-only\n");
	if (dwCaps & 0x00002000)
		printf("    - DDSCAPS_ZBUFFER: Surface is a Z-buffer\n");
	if (dwCaps & 0x00004000)
		printf("    - DDSCAPS_OWNDC: Surface has its own device context (DC)\n"
		);
	if (dwCaps & 0x00008000)
		printf("    - DDSCAPS_LIVEVIDEO: Surface is receiving live video\n");
	if (dwCaps & 0x00010000)
		printf("    - DDSCAPS_HWCODEC: Hardware-accelerated codec surface\n");
	if (dwCaps & 0x00020000)
		printf("    - DDSCAPS_MODEX: ModeX surface\n");
	if (dwCaps & 0x00040000)
		printf("    - DDSCAPS_MIPMAP: Surface is one level of a mipmap chain\n"
		);
	if (dwCaps & 0x00080000)
		printf("    - DDSCAPS_RESERVED2\n");
	if (dwCaps & 0x00100000)
		printf("    - DDSCAPS_ALLOCONLOAD: Allocated when loaded\n");
	if (dwCaps & 0x00200000)
		printf("    - DDSCAPS_VIDEOPORT: Surface is part of a video port\n");
	if (dwCaps & 0x00400000)
		printf("    - DDSCAPS_LOCALVIDMEM: Surface is in local video memory\n");
	if (dwCaps & 0x00800000)
		printf("    - DDSCAPS_NONLOCALVIDMEM: Surface is in non-local video "
			   "memory\n");
	if (dwCaps & 0x01000000)
		printf("    - DDSCAPS_STANDARDVGAMODE: Supports standard VGA mode\n");
	if (dwCaps & 0x02000000)
		printf("    - DDSCAPS_OPTIMIZED: Optimized surface\n");
	if (dwCaps & 0x04000000)
		printf("    - DDSCAPS2_HARDWAREDEINTERLACE: Supports hardware "
			   "deinterlacing\n");
	if (dwCaps == 0)
		printf("    - (No specific capabilities set)\n");
}
...
void printDDCaps(const DDCAPS* caps, const char* label) {
	...
	if (caps->dwCaps & DDCAPS_3D)
		printf("    - Supports 3D acceleration\n");
	if (caps->dwCaps & DDCAPS_BLT)
		printf("    - Supports BLT (Bit Block Transfer)\n");
	if (caps->dwCaps & DDCAPS_BLTQUEUE)
		printf("    - Supports queued BLTs\n");
	if (caps->dwCaps & DDCAPS_BLTCOLORFILL)
		printf("    - Supports color fill BLTs\n");
	if (caps->dwCaps & DDCAPS_GDI)
		printf("    - Can render with GDI\n");
	if (caps->dwCaps & DDCAPS_OVERLAY)
		printf("    - Supports hardware overlay\n");
	if (caps->dwCaps & DDCAPS_OVERLAYCANTCLIP)
		printf("    - Cannot clip overlays\n");
	if (caps->dwCaps & DDCAPS_OVERLAYSTRETCH)
		printf("    - Supports overlay stretching\n");
	if (caps->dwCaps & DDCAPS_ALPHA)
		printf("    - Supports alpha blending\n");
	if (caps->dwCaps & DDCAPS_COLORKEY)
		printf("    - Supports colorkeying\n");
	if (caps->dwCaps & DDCAPS_CANCLIP)
		printf("    - Supports surface clipping\n");
	if (caps->dwCaps & DDCAPS_CANBLTSYSMEM)
		printf("    - Can BLT from system memory\n");
	if (caps->dwCaps & DDCAPS_READSCANLINE)
		printf("    - Can read the current scanline\n");
	...
	if (caps->dwCaps2 & DDCAPS2_CANBOBINTERLEAVED)
		printf("    - Supports interleaved bob deinterlacing\n");
	if (caps->dwCaps2 & DDCAPS2_CANBOBNONINTERLEAVED)
		printf("    - Supports non-interleaved bob deinterlacing\n");
	if (caps->dwCaps2 & DDCAPS2_WIDESURFACES)
		printf("    - Supports wide surfaces\n");
...
BOOL MxDirect3D::D3DCreate() {
	...
	result =
		DirectDraw()->QueryInterface(IID_IDirect3D2, (LPVOID*) &m_pDirect3d);
	if (result != DD_OK) {
...
BOOL MxDirect3D::D3DSetMode() {
	if (m_assignedDevice->m_flags & MxAssignedDevice::c_hardwareMode) {
		...
		if (m_bOnlySoftRender) {
			Error(
				"Failed to place vital surfaces in video memory for hardware "
				"driver",
				DDERR_GENERIC
			);
			return FALSE;
		...
		if (m_assignedDevice->m_desc.dpcTriCaps.dwTextureCaps &
	...
	HRESULT result = m_pDirect3d->CreateDevice(
		m_assignedDevice->m_guid,
		m_pBackBuffer,
		&m_pDirect3dDevice
	);
	if (result != DD_OK) {
	...
	if (backBuffer->Lock(NULL, &desc, DDLOCK_WAIT, NULL) == DD_OK) {
		...
		for (int i = mode.height; i > 0; i--) {
			memset(
				surface,
				0,
				mode.width * desc.ddpfPixelFormat.dwRGBBitCount / 8
			);
			surface += desc.lPitch;
		}
...
BOOL MxDirect3D::SetDevice(
	...
	for (list<MxDriver>::iterator it = p_deviceEnumerate.m_list.begin();
		 it != p_deviceEnumerate.m_list.end();
		 it++) {
		MxDriver& driver = *it;
		...
		if (&driver == p_driver) {
			...
			if (driver.m_guid) {
				...
				memcpy(
					assignedDevice->m_deviceInfo->m_guid,
					driver.m_guid,
					sizeof(GUID)
				);
			}

			assignedDevice->m_deviceInfo->m_count =
				driver.m_displayModes.size();

			if (assignedDevice->m_deviceInfo->m_count > 0) {
				assignedDevice->m_deviceInfo->m_modeArray =
					new DeviceModesInfo::Mode[assignedDevice->m_deviceInfo
												  ->m_count];

				int j = 0;
				for (list<MxDisplayMode>::iterator it2 =
						 driver.m_displayModes.begin();
					 it2 != driver.m_displayModes.end();
					 it2++) {
					assignedDevice->m_deviceInfo->m_modeArray[j].width =
						(*it2).m_width;
					assignedDevice->m_deviceInfo->m_modeArray[j].height =
						(*it2).m_height;
					assignedDevice->m_deviceInfo->m_modeArray[j].bitsPerPixel =
						(*it2).m_bitsPerPixel;
					j++;
				}
			}
			...
			for (list<Direct3DDeviceInfo>::iterator it2 =
					 driver.m_devices.begin();
				 it2 != driver.m_devices.end();
				 it2++) {
				...
				memcpy(
					&assignedDevice->m_guid,
					device.m_guid,
					sizeof(assignedDevice->m_guid)
				);
				...
				memcpy(
					&assignedDevice->m_desc,
					desc,
					sizeof(assignedDevice->m_desc)
				);
