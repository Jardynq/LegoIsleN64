...
BOOL MxDirect3D::Create(
	...
	if (!ret) {
		success = FALSE;
		log_error("Failed to create mx direct draw\n");
	}
	if (!D3DCreate()) {
		success = FALSE;
		log_error("Failed to create mx direct 3d\n");
	}
	if (!D3DSetMode()) {
		success = FALSE;
		log_error("Failed to set mx direct 3d mode\n");
	}
...
BOOL MxDirect3D::D3DCreate() {
	...
	if (result != DD_OK) {
		log_error("Creation of IDirect3D failed", result);
		return FALSE;
	}
	return TRUE;
}
...
BOOL MxDirect3D::D3DSetMode() {
	if (m_assignedDevice->m_flags & MxAssignedDevice::c_hardwareMode) {
		log_info("Using hardware renderer\n");
	...
	if (result != DD_OK) {
		log_error("Create D3D device failed", result);
		return FALSE;
	}
	...
	if (backBuffer->Lock(NULL, &desc, DDLOCK_WAIT, NULL) == DD_OK) {
		...
		log_error("Back buffer lock failed\n");
	}
	...
	if (m_bFullScreen) {
		...
		if (frontBuffer->Lock(NULL, &desc, DDLOCK_WAIT, NULL) == DD_OK) {
			...
			log_error("front buffer lock failed\n");
		}
	}
